...
4. *With DeepSeek later*: AutoNuggetizer-style nugget assignment, ALCE NLI-style support (quote → claim), RAGAS faithfulness; abstention scored as a **coverage–risk curve** (answer rate vs error rate, AUC) on answerable + unanswerable questions, conditioned on our sufficiency label so retrieval and generation failures are separated. Calibrate the judge with ~150 hand labels (ARES PPI).
