...
Store learned-sparse impact vectors in an inverted index next to HNSW dense vectors in one engine and fuse server-side in a single query (Vespa `wand`+`nearestNeighbor`, OpenSearch/Elasticsearch `sparse_vector`+`knn`, Qdrant `prefetch`+fusion, Milvus `hybrid_search`, Infinity), instead of bm25s + numpy + scipy-CSR fused in Python.

**Why it fits this project**

- Exp 12 adds a third (SPLADE-style) leg; three in-process indexes at 1M chunks are memory-heavy and brute-force dense scoring becomes the latency floor (*unverified*: >100 ms/query on 4 cores).
- A unified engine gives persistence, filters (doc type, year, language) inside both legs, WAND/MaxScore pruning, one round-trip.
- Fusion itself is not the problem: convex/RRF over ≤300 candidates costs microseconds, and exp 03/14 tune weights and LTR offline, which most engines cannot express.

**Evidence** (checked 2026-09-25; *unverified* where marked)

| Engine | Sparse storage | One-query hybrid | Latency evidence |
|---|---|---|---|
| Vespa | `weightedset<int>`, `wand`, `rawScore` | YQL `OR`/`rank()`; phased ranking, `reciprocal_rank`, `normalize_linear`, free expressions | WAND scores ~2.2 % of MS MARCO vs 89 % for OR; no ms figures [1][2][3] |
| OpenSearch 3.3 | `rank_features` (exact) or `sparse_vector` (SEISMIC ANN) | `hybrid` + normalization processor | two-phase P99 doc-only 198→124 ms, bi-encoder 617→122 ms (2.7–5.4M docs, 3×m5.4xlarge, 2025-06); SEISMIC, 1.29 B docs: P50 11 ms vs exact 109 vs BM25 28, recall@10 90 %, ~1 GB RAM/1M docs (2025-10) [4][5] |
...
Extrapolating from 8.8M, an exact impact index over 1M chunks should answer in ~10–40 ms single-threaded (*unverified*); posting density drives latency, hence every engine's pruning knobs.

**How we would implement it**

1. Only if exp 12 shows the sparse leg pays: Qdrant (single Rust binary) with `text_dense` (e5-base, HNSW) + `text_sparse` (SPLADE-fr, no IDF modifier) + payload filters; French-stemmed BM25 stays in Tantivy/bm25s.
...
- Latency/memory: HNSW replaces brute-force dense, WAND replaces CSR — real at 1M chunks, but Tantivy + an ANN index (ideas 16–18) give the same serverless.
- Cost: a service to run, three representations to re-ingest, 2–4 dev days; Vespa/OpenSearch are oversized for a 4-core box.

**Risks / open questions**

- Fusion locked to engine primitives; tuned convex weights and exp 14 LTR stay in Python.
...
**skip** — Python-side fusion is not the bottleneck and the engines add no ranking quality; revisit Qdrant only if exp 12 proves a learned-sparse leg and brute-force dense scoring at 1M chunks is the measured floor.
