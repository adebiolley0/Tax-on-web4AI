...
- We already store an evidence quote per B/C question; that is exactly the span annotation LegalBench-RAG (character ranges) and LLeQA (paragraph rationales) use, so chunk-level attribution metrics are nearly free.
- C already has "several acceptable ids" and 0.5-weighted secondary docs, i.e. an informal graded-relevance scheme; formalising it makes nDCG meaningful.
...
- LegalBench-RAG: 6,858 queries, 79M chars, ground truth = character spans; precision@k/recall@k for k=1…64; small chunks (~500 chars) raise precision (verified: spans, P/R; chunk-size finding from memory). https://github.com/zeroentropy-ai/legalbenchrag
- COLIEE: task 1 case retrieval (micro-F1), task 3 statute retrieval over Japanese Civil Code (macro-F2), tasks 2/4 entailment (accuracy) — site fetch failed, from memory. https://coliee.org
...
- LePaRD: passage-level precedent prediction, citation-derived labels (noisy, no negatives). https://arxiv.org/abs/2311.09356
- LeCaRD / LeCaRDv2 (CAIL family): 4-level graded relevance, nDCG@10/20/30 + P@k — the graded-relevance model. AILA (FIRE 2019): 50 queries, statute + precedent retrieval, MAP/BPREF. ECtHR/LexGLUE and EUR-Lex are classification (F1), not retrieval; ECtHR-PCR (2024) is the retrieval variant. (all unverified)
- BEIR has no legal subset (nDCG@10 primary, R@100 secondary); MTEB includes AILACasedocs/AILAStatutes, LegalBench-ConsumerContractsQA, GerDaLIR, LeCaRDv2 at nDCG@10 (unverified list). https://arxiv.org/abs/2104.08663
...
3. Chunk-level metric (LegalBench-RAG style): a retrieved chunk is relevant if it overlaps a gold span; report span-P@k / span-R@k for k=5, 20.
...
5. Document in EXPERIMENTS.md that B is "BSARD-like" and C is "document-level, graded".

**Expected gain and cost**

No retrieval gain; comparability and diagnostic power: R@k separates "retriever misses" from "reranker misorders" (on C, R@10 0.891 vs H@1 0.609 already says most loss is ordering). BSARD gives a 222-question, third-party-labelled sanity check of our French normalisation and hybrid stack. Cost: ~1 day of harness work, ~1 h to embed 22.6k articles with e5-small, ~1.5 h of reranking on the 4-core box.
...
- Migrating grades changes existing leaderboard numbers; keep the old MRR column untouched and version the metric set (`metrics_v2`).
- Span offsets break whenever a document is re-parsed (idea 61); store quote text and re-derive offsets at load time.
- Licence: BSARD is CC-BY-NC-SA — fine for evaluation, not for shipping.
