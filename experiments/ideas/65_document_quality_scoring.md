...
Corpus C (exp 09) inventoried exactly this noise: *aperçu documentaire* index pages, TOC-only documents, 1–2 kB stubs, Dutch bodies flagged `fr` (≈ half the rulings), abrogated codes beside current ones, banking law and waste decrees, per-region/year indexation notices. `MYFIN_ARBORESCENCE.md` § Classification already holds a human include/exclude decision per Fisconet+ section — free weak supervision. Exp 08 showed boilerplate removal alone gives BM25 +0.03 MRR / +0.075 hit@1, so ingestion hygiene is a measured lever. CPU-only, model-free, minutes over 21k documents.

**Evidence**

- Gopher (Rae et al. 2021) rules: word-count bounds, mean word length 3–10, symbol-to-word ratio < 0.1, < 90 % bullet lines, < 30 % ellipsis-ended lines, ≥ 80 % alphabetic words, stop-word presence. https://arxiv.org/abs/2112.11446
...
- Legal corpora (Pile of Law, MultiLegalPile) apply source-level curation, language ID and dedup before release. https://arxiv.org/abs/2207.00220 · https://arxiv.org/abs/2306.02069
- Retrieval priors: Bendersky, Croft & Diao (WSDM 2011) "Quality-biased ranking" — readability, stop-word fraction, entropy as document priors improve web retrieval; Kraaij et al. (SIGIR 2002) priors for entry-page search. https://dl.acm.org/doi/10.1145/1935826.1935849 · https://dl.acm.org/doi/10.1145/564376.564383
- Engines expose the hook: Elasticsearch `rank_feature` / `function_score`, Vespa rank-profile document features. https://www.elastic.co/docs/reference/query-languages/query-dsl/query-dsl-rank-feature-query

(Thresholds quoted from memory; WebSearch quota exhausted — verify before coding.)
...
5. QA report per crawl: `quality` histogram per document type, 50 lowest-scored kept and 50 highest-scored dropped documents for spot review, drift vs previous crawl — a markdown/CSV artefact from the harness.

**Expected gain and cost**

Hard drop ≈ 10–20 % of corpus C; +0.02–0.05 MRR on C from fewer junk candidates in the reranker's top-30, more on questions whose target has a low-content twin. Ingestion cost seconds; engineering 1–2 days for rules + report, +1 day for the classifier; only new dependency is a language detector.
...
- Overlaps with ideas 26 (dedup) and 41 (temporal validity); run this first, they consume its metadata.
