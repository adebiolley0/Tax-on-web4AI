...
Instead of stripping amendment notes as noise (+0.03 BM25 MRR), parse them at ingestion into a
`validity` record per provision — `{scope, event, date|ay, source_act, mb_date, numac, status}` — and use
it at query time: anchor queries to the current income year, soft-demote non-applicable versions, answer
"depuis quand ?" from the record, badge future and abrogated law. Topic 25 designs the versions table;
this topic supplies the extractor and the query-side behaviour.

**Why it fits this project**

- The notes are formulaic. Count over 8,061 `code_et_legislation` files: 529 × "Art. N est applicable à partir de l'exercice d'imposition N (art. …, L dd.mm.yyyy – M.B. …; Numac …)", ~320 × "est abrogé à partir du dd.mm.yyyy / de l'exercice d'imposition N", ~150 × "entre en vigueur le dd.mm.yyyy | le jour de sa publication", ~60 × "applicable N jours après publication", plus event tails ("aux revenus attribués ou mis en paiement à partir du …", "aux périodes imposables clôturées à partir du …"). Succession/registration codes: "Article N (applicable à partir du dd.mm.yyyy)" and "DROIT FUTUR (à partir du …) :" blocks (already `@date`-suffixed by `parse_pdfs.py`); regional decrees: "modifié par l'art. N du décret du … Texte entre en vigueur le …".
- Fisconet+ gives only document-level `effectiveDate`/`historyLink`; MyMinfin markdown has no dates — the text is the only source.
- Users think in income years, the corpus in assessment years (AY = income year + 1).
...
- Akoma Ntoso: `<eventRef date source type=generation|amendment|repeal>`, `<timeInterval start end>`, fragment `@start/@end/@startEfficacy/@endEfficacy` — our field set, at fragment level. https://docs.oasis-open.org/legaldocml/akn-core/v1.0/os/part1-vocabulary/akn-core-v1.0-os-part1-vocabulary.html
...
- HeidelTime-fr normalises dates, not legal events; a domain grammar is needed. Justel's "Entrée en vigueur" header is not recomputed after amendment (unverified).
...
Normalisation: `AY N` → income year N−1; "N jours après publication" → `mb_date + N`; "jour de sa publication" → `mb_date`; bare `(abrogé)` → status only. Keep every note (one per paragraph); status = latest event ≤ today, `future` if the earliest event is later. Validate on a 200-note hand-checked sample; unparsed notes fall back to Fisconet `effectiveDate`.

Query side: regex for `revenus YYYY`, `exercice d'imposition YYYY`, `dd.mm.yyyy`, "depuis quand / encore applicable / droit futur"; default anchor = current income year; soft α scoring as in topic 25; "depuis quand" answered from the record with its source act; each hit badged *en vigueur (revenus 2026)* / *droit futur (01.01.2028)* / *abrogé depuis …*. No LLM needed; DeepSeek later resolves fuzzy anchors and explains version differences.

**Expected gain and cost**

Retrieval MRR on the present sets: ≈ 0 (questions accept any edition). Gain is answer correctness: applicable-version rate ~0 % → >90 % on a new ~30-question year-anchored set (literature: 98 %), correct "depuis quand" answers, no silent future-law answers. Cost: 1–2 days grammar + validation, half a day query-side; seconds to run over 21k docs.

**Risks / open questions**

Paragraph-level scopes (§ 3 future, rest current) need sub-article versions or a "partly future" flag; retroactive laws break monotone intervals; corporate tax follows accounting periods, not calendar years; circulaires/rulings have no notes (`documentDate` only); OCR-noised headers ("A rticle 48 2 ( applicable …)") need tolerant whitespace.

**Verdict**

**try-now** — a one-day regex grammar covers the notes, version-aware retrieval is the difference between 0 % and 98 % applicable-version answers in the literature, and it is the missing input of topic 25.
...
- Local: `experiments/08_corpus_b_cleanup/README.md`, `experiments/00_pdf_parsing/README.md`, `experiments/ideas/25_temporal_versioned_indexing.md`
